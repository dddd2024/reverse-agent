...
static void build_key76(const uint8_t prefix8[PREFIX_LEN], uint8_t key[76]) {
...
int main(int argc, char **argv) {
    ...
    const char *base_hex = "e05e579fca169e80";
        ...
        if (trial.exact_prefix_len >= 4) {
            ...
            if (better_entry(&trial, &best)) {
                best = trial;
                {
                    ...
                    bytes_to_hex(best.lhs, PREFIX_BYTES, lhs_hex);
                    printf(
                        "BEST exact=%d dist6=%d dist10=%d cand=%s lhs=%s evals=%llu\n",
...

    {
        ...
        fprintf(
            ...
            "  \"seed\": %llu,\n"
            "  \"evaluations\": %llu,\n"
            "  \"best\": {\"cand8_hex\":\"%s\",\"candidate_hex\":\"%s414141414141\",\"lhs_prefix_hex\":\"%s\",\"exact_prefix_len\":%d,\"distance6\":%d,\"distance10\":%d}\n"
            "}\n",
