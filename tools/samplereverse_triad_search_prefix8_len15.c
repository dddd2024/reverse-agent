...
static void build_key76(const uint8_t prefix8[8], uint8_t key[80]) {
...
int main(int argc, char **argv) {
    ...
    if (argc != 5) {
        fprintf(stderr, "usage: %s p0 p1 p2 base_hex16\n", argv[0]);
        return 2;
    }
    p0 = atoi(argv[1]);
    p1 = atoi(argv[2]);
    p2 = atoi(argv[3]);
    if (
        p0 < 0 || p0 >= 8 || p1 < 0 || p1 >= 8 || p2 < 0 || p2 >= 8 ||
        p0 == p1 || p0 == p2 || p1 == p2
    ) {
        fprintf(stderr, "invalid positions\n");
        return 2;
    }
    if (!parse_hex_seed(argv[4], base)) {
