...
static void print_row(const char *label, const Row *row) {
    ...
    printf(
        "%s exact=%d dist4=%d dist6=%d dist10=%d cand8=%s raw=%s combo=[%d,%d,%d,%d,%d]\n",
        label,
        row->exact,
        row->dist4,
        row->dist6,
...
int main(int argc, char **argv) {
    ...
    best.nibbles[0] = best.nibbles[1] = best.nibbles[2] = best.nibbles[3] = best.nibbles[4] = -1;
            ...
            for (c = b + 1; c < NIBBLE_COUNT; ++c) {
                for (d = c + 1; d < NIBBLE_COUNT; ++d) {
                    for (e = d + 1; e < NIBBLE_COUNT; ++e) {
                        ...
                        if ((eval_count % 50000000ULL) == 0ULL) {
                            printf("checkpoint evals=%llu combo=[%d,%d,%d,%d,%d]\n",
                                (unsigned long long)eval_count, a, b, c, d, e);
                            print_row("CUR", &best);
                            fflush(stdout);
                        }
                    }
